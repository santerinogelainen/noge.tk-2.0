...
if (!empty($_POST) && login_check($mysqli)) {
  ...
  if ($query = $mysqli->prepare("SELECT id FROM imageposts WHERE id=?")) {
  ...
  if ($exists) {
    $sql = "UPDATE imageposts SET `page_title`=?, post=?, public=? WHERE id=?";
  } else {
    $sql = "INSERT INTO imageposts (`page_title`, post, public, id) VALUES (?, ?, ?, ?)";
  }
  ...
  if ($query = $mysqli->prepare($sql)) {
    ...
    if ($query->execute()) {
      if ($public == 0) {
        ...
        echo "Saved and published. <a target='_blank' href='" . BASE_URL . "image?id=" . $id . "'>Preview</a>";
      }
    } else {
      echo "Mysql execute error.";
      exit;
