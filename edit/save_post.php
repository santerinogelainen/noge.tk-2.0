...
if (!empty($_POST) && login_check($mysqli)) {
  ...
  if ($query = $mysqli->prepare("SELECT id FROM posts WHERE id=?")) {
  ...
  if ($exists) {
    $sql = "UPDATE posts SET title=?, `page_title`=?, post=?, public=? WHERE id=?";
  } else {
    $sql = "INSERT INTO posts (title, page_title, post, public, id) VALUES (?, ?, ?, ?, ?)";
  }
  
  //insert/update
  if ($query = $mysqli->prepare($sql)) {
    $query->bind_param("sssii", $title, $page_title, $post, $public, $id);
    if ($query->execute()) {
      if ($public == 0) {
        echo "Saved as a draft.";
      } else {
        echo "Saved and published. <a target='_blank' href='" . BASE_URL . "post?id=" . $id . "'>Preview</a>";
      }
    } else {
      echo "Mysql execute error.";
      exit;
